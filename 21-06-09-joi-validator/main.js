...
(async () => {
  const schema = Joi.array().items(
    Joi.object({
      ...
      type: Joi.string().valid('audio', 'video', 'image', 'sticker', 'text', 'subtitle', 'frame', 'shader', 'title').required(), // track 类型
      ...
      items: Joi.array().items(
        Joi.object({
          ...
          operations: Joi.array().items(
            Joi.object().keys({
              type: Joi.string().valid('image_mirror',
                'image_rotate',
                'image_filter_normal',
                'image_filter_lut',
                'image_crop',
  ...
  for (const tracks of tracksList) {
    try {
      await schema.validateAsync(tracks, {
